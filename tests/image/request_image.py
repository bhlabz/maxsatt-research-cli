...
def process(geojson_path, date_range, output_dir):
    ...
    scenes = search_scenes(polygon, date_range)
    ...
    for scene in scenes:
        ...
        # Extract scene info from scene_id to build S3 prefix
        # Scene ID format: S2A_22KBC_20230629_0_L2A
        # We need: tiles/22/K/BC/2023/6/29/0/
        ...
        if len(parts) >= 4:
            ...
            if len(tile_info) >= 5:
                ...
                # List available files in this directory
                print("Checking available files in S3...")
                list_s3_directory(S3_BUCKET, s3_prefix)
                
                # Now process only the bands that have corresponding files
                for band, asset_info in BAND_MAP.items():
                    ...
                    # Construct the correct file path with resolution folder
                    ...
                    try:
                        ...
                        with tempfile.NamedTemporaryFile(suffix=".jp2") as tmp_jp2:
                            ...
                            try:
                                ...
                                if cropped is None:
                                    print(f"Skipping {band} - no intersection with raster")
                                    continue
                            except Exception as e:
